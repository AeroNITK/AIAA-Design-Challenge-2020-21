...
function Aircraft = Aero(Aircraft)
    ...
    %% L/D Estimation:
    K_LD = 11;  % Factor from Raymer Pg 40 Eq: 3.12
    ...
    Aircraft.Performance.L_by_D = C_L / (C_D_0 + C_L^2 / (pi * Aircraft.Wing.Aspect_Ratio * 0.75)); % 0.75 is the 'e' assumed here.
    
    CGRP = 18.97 * n_p / (Aircraft.Performance.WbyS^0.5 * Aircraft.Performance.WbyP);
    
    Aircraft.Performance.CGR = CGRP * sqrt(C_L) - 1 / Aircraft.Performance.L_by_D;
    ...
    Aircraft.Performance.cruise2_range = Aircraft.Performance.total_range - climb_range; % in nautical miles
